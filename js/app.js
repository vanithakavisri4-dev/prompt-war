...
const App = (() => {
  ...
  function bindEvents() {
    // Onboarding
    $('#onboarding-form')?.addEventListener('submit', e => { e.preventDefault(); handleOnboarding(); });
    // Navigation
    ...
    $('#btn-dismiss-emergency')?.addEventListener('click', () => { $('#emergency-overlay').style.display = 'none'; });
    // Chat
    $('#chat-form')?.addEventListener('submit', e => { e.preventDefault(); handleChat(); });
    // Quick actions
    $('#qa-food')?.addEventListener('click', () => { switchView('concierge'); simulateChat('Where can I find food with the shortest wait?'); });
    $('#qa-restroom')?.addEventListener('click', () => { switchView('concierge'); simulateChat('Where is the nearest restroom?'); });
    $('#qa-merch')?.addEventListener('click', () => { switchView('concierge'); simulateChat('Where is the merchandise store?'); });
    $('#qa-exit')?.addEventListener('click', () => { switchView('concierge'); simulateChat('What is the fastest exit?'); });
    $('#qa-medical')?.addEventListener('click', () => { switchView('concierge'); simulateChat('I need medical help'); });
    $('#qa-seat')?.addEventListener('click', () => { switchView('concierge'); simulateChat('How do I get back to my seat?'); });
    // Flow
    $('#btn-regenerate-flow')?.addEventListener('click', renderFlow);
    // Map layers
    $$('.map-ctrl-btn').forEach(btn => btn.addEventListener('click', () => {
      $$('.map-ctrl-btn').forEach(b => { b.classList.remove('active'); b.setAttribute('aria-pressed', 'false'); });
    ...
    // Settings
    $('#setting-theme')?.addEventListener('change', e => AccessibilityService.setTheme(e.target.value));
    $('#setting-font-scale')?.addEventListener('input', e => AccessibilityService.applyFontScale(e.target.value / 100));
    $('#setting-reduce-motion')?.addEventListener('change', e => AccessibilityService.toggleReduceMotion(e.target.checked));
    // Accessibility buttons
    $('#btn-high-contrast')?.addEventListener('click', () => AccessibilityService.toggleHighContrast());
    $('#btn-font-size')?.addEventListener('click', () => AccessibilityService.increaseFontSize());
    // Clock
    setInterval(() => { const el = $('#current-time'); if (el) el.textContent = ArenaUtils.formatTime(); }, 1000);
  }

  function handleOnboarding() {
    const form = $('#onboarding-form');
    if (!validateForm(form)) { showToast('Please fill all required fields', 'warning'); return; }
  ...
  function drawHeatmapCanvas(snap) {
    if (!_hmCtx) return;
    const w = _hmCanvas.clientWidth, h = _hmCanvas.clientHeight;
    _hmCtx.clearRect(0, 0, w, h);
    _hmCtx.fillStyle = '#0d1117'; _hmCtx.fillRect(0, 0, w, h);
    // Stadium outline ellipse
    _hmCtx.beginPath(); _hmCtx.ellipse(w/2, h/2, w*.44, h*.44, 0, 0, Math.PI*2);
    _hmCtx.strokeStyle = 'rgba(108,99,255,0.3)'; _hmCtx.lineWidth = 2; _hmCtx.stroke();
    _hmCtx.beginPath(); _hmCtx.ellipse(w/2, h/2, w*.28, h*.28, 0, 0, Math.PI*2);
    _hmCtx.strokeStyle = 'rgba(0,212,255,0.2)'; _hmCtx.lineWidth = 1; _hmCtx.stroke();
    // Field
    _hmCtx.fillStyle = 'rgba(16,185,129,0.08)';
    _hmCtx.beginPath(); _hmCtx.ellipse(w/2, h/2, w*.18, h*.18, 0, 0, Math.PI*2); _hmCtx.fill();
  ...
  function renderPredictions() {
    ...
    list.innerHTML = preds.map(p => `
      <li class="prediction-item ${p.type}" role="listitem">
  ...
  function renderFlow() {
    ...
    const profile = { section: _state.section, accessibility: _state.accessibility };
    const flow = FlowOptimizer.generateFlow(profile, snapshot);
    timeline.innerHTML = flow.items.map((item, i) => `
      <div class="flow-item ${item.status}" role="listitem" style="animation-delay:${i*0.1}s">
        <span class="flow-item-time">${sanitize(item.time)}</span>
        <h3 class="flow-item-title">${item.icon} ${sanitize(item.label)}</h3>
        <p class="flow-item-desc">📍 ${sanitize(item.zone)} · ${sanitize(item.duration)} · Density: ${item.density}%</p>
  ...
  function handleCreateGroup() {
    ...
    const code = FirebaseService.createGroup(name, { name: _state.user, section: _state.section });
  ...
  function handleJoinGroup() {
    ...
    const group = FirebaseService.joinGroup(code, { name: _state.user, section: _state.section });
    if (group) { _state.groupCode = code; showGroupMembers(code); showToast('Joined group!', 'success'); }
  ...
  function showGroupMembers(code) {
    ...
    list.innerHTML = group.members.map((m, i) => `
  ...
  function handleFindMeetup() {
    ...
    if (!group || !group.members.length) { showToast('No group members', 'warning'); return; }
    const sections = group.members.map(m => m.section || 'north-lower');
    const result = FlowOptimizer.findMeetingPoint(sections, CrowdEngine.getSnapshot());
  ...
  function renderAlerts() {
    ...
    if (!list) return;
    const alerts = [
      { icon: '📊', title: 'Crowd density increasing', desc: 'North concourse reaching 70% capacity. Consider alternate routes.', type: 'warning', time: '2 min ago' },
      { icon: '🍔', title: 'Food Court NE - Short lines!', desc: 'Wait times under 3 minutes at the northeast food court.', type: 'success', time: '5 min ago' },
      { icon: '🔀', title: 'Your flow updated', desc: 'AI has optimized your activity timeline based on latest crowd data.', type: '', time: '8 min ago' },
      { icon: '🏟️', title: 'Event starting soon', desc: 'Match kickoff in 15 minutes. Please head to your seats.', type: '', time: '12 min ago' },
      { icon: '⚠️', title: 'Gate C congested', desc: 'Heavy crowd at south gate. Use Gate B or D for faster entry.', type: 'danger', time: '18 min ago' },
    ];
    list.innerHTML = alerts.map(a => `
      <div class="alert-item ${a.type}" role="listitem">
  ...
  function triggerEmergency() {
    ...
    route.innerHTML = `🚪 Nearest Exit: <strong>${bestGate ? bestGate.name : 'Gate B'}</strong><br>` +
      `📊 Current density: ${bestGate ? Math.round(bestGate.density * 100) : 25}%<br>` +
      `➡️ Follow illuminated emergency signs to the ${bestGate ? bestGate.name.split('(')[1]?.replace(')','') : 'East'}`;
    AccessibilityService.announce('Emergency evacuation mode activated. Follow the displayed exit route.');
  }
